...
export function calculateNextDueDate(task: Task): Date | null {
  ...
  // If no time is scheduled, use start of day (midnight)
  ...
  const findNextFromDate = (fromDate: Date, fromCompletion: boolean = false, maxDepth: number = 10): Date | null => {
    if (maxDepth <= 0) {
    ...
    switch (schedule.repeat) {
      ...
      case 'weekly': {
        ...
        if (daysUntilTarget === 0 && (fromCompletion || candidate.getTime() <= fromDate.getTime())) {
          daysUntilTarget = 7;
        }
      ...
      case 'biweekly': {
        ...
        const daysFromStart = Math.floor((candidate.getTime() - startDateDay.getTime()) / (1000 * 60 * 60 * 24));
      ...
      case 'monthly': {
        ...
        if (fromCompletion || candidate.getTime() <= fromDate.getTime()) {
          ...
          candidate.setDate(targetDay);
          const nextDaysInMonth = new Date(candidate.getFullYear(), candidate.getMonth() + 1, 0).getDate();
      ...
      case 'yearly': {
        ...
        if (fromCompletion || candidate.getTime() <= fromDate.getTime()) {
          ...
          if (targetMonth === 1 && targetDay === 29 && !isLeapYear(candidate.getFullYear())) {
            candidate.setDate(28);
          }
          candidate = setTimeOnDate(candidate);
        }
  ...
  const searchStart = searchDate.getTime() > startDate.getTime() ? searchDate : startDate;
